...
// Captura o código de indicação (?ref=CODIGO) em qualquer página do site,
// não só na home — assim funciona mesmo se o link compartilhado for de
// um produto específico.
(function capturarIndicacao() {
  const ref = new URLSearchParams(window.location.search).get('ref');
  if (ref) localStorage.setItem('codigo_indicacao', ref.toUpperCase());
})();
...
// A navbar e o footer são injetados por JS, então os textos editáveis
// dentro deles precisam ser aplicados DEPOIS da injeção — e de novo
// quando a navbar é recriada no login. Como este arquivo é um script
// comum (não module), o loader entra por import() dinâmico.
// Se falhar, cada elemento simplesmente mantém o texto padrão do HTML.
function aplicarTextosEm(el) {
  if (!el) return;
  import(`${BASE}/js/textos-loader.js`)
    .then(m => m.aplicarTextosSite(el))
    .catch(() => {});
}
...
function navbarHTML(user, isAdmin = false) {
  ...
  <nav class="navbar">
    <div class="navbar-inner">
      ...
      <form class="search-form" onsubmit="handleSearch(event)" style="position:relative">
        <input type="search" id="search-input" placeholder="Buscar produtos, marcas e categorias..." data-texto="navbar_busca_placeholder" data-texto-attr="placeholder" autocomplete="off" oninput="buscarSugestoes(this.value, 'search-suggest')">
        <button type="submit">🔍</button>
        <div class="search-suggest" id="search-suggest"></div>
      </form>
      <div class="nav-actions">
        <button type="button" class="search-toggle-btn" onclick="toggleMobileSearch()" aria-label="Buscar">🔍</button>
    ...
    <form class="mobile-search-bar" id="mobile-search-bar" onsubmit="handleSearch(event)" style="position:relative">
      <input type="search" id="mobile-search-input" placeholder="Buscar produtos, marcas e categorias..." data-texto="navbar_busca_placeholder" data-texto-attr="placeholder" autocomplete="off" oninput="buscarSugestoes(this.value, 'mobile-search-suggest')">
      <button type="submit">🔍</button>
      <div class="search-suggest" id="mobile-search-suggest"></div>
    </form>
    ...
    <div class="footer-inner">
      <div class="footer-grid">
        <div>
          ...
          <p class="footer-desc"><span data-texto="footer_descricao_1">A loja de eletrônicos mais rápida do Brasil.</span><br><span data-texto="footer_descricao_2">Pra ontem, pra agora, pra você.</span></p>
        ...
        <div>
          <p class="footer-col-title" data-texto="footer_col_comprar">Comprar</p>
        ...
        <div>
          <p class="footer-col-title" data-texto="footer_col_ajuda">Ajuda</p>
          <div class="footer-links">
            <a href="${BASE}/conta.html" data-texto="footer_link_conta">Minha Conta</a>
            <a href="${BASE}/conta.html" data-texto="footer_link_rastrear">Rastrear Pedido</a>
            <a href="https://wa.me/5555984566918" target="_blank" rel="noopener" data-texto="footer_link_devolucoes">Trocas e Devoluções</a>
          </div>
        </div>
        <div>
          <p class="footer-col-title" data-texto="footer_col_suporte">Suporte</p>
          <div class="footer-links">
      ...
      <div class="footer-bottom">
        <span data-texto="footer_copyright">© 2024 Zipshop. Todos os direitos reservados.</span>
...
function handleSearch(e) {
  e.preventDefault();
  const q = (document.getElementById('search-input')?.value || document.getElementById('mobile-search-input')?.value || '').trim();
  if (q) window.location.href = `${BASE}/produtos.html?q=${encodeURIComponent(q)}`;
}

// Busca "de verdade" com sugestões ao vivo: consulta o banco enquanto
// a pessoa digita (com debounce), em vez de só filtrar o que já tinha
// carregado na tela.
...
async function getSearchClient() {
  ...
  _searchClient = createClient('https://kkliwdphrdbguclsvxcw.supabase.co', 'sb_publishable_OMXANLsZK98fPWygp8ZHfA_xzSpWgYb');
  return _searchClient;
}

window.buscarSugestoes = function(termo, containerId) {
  ...
  _searchDebounce = setTimeout(async () => {
    const sb = await getSearchClient();
    const termoSeguro = q.replace(/[,()]/g, ' ').trim();

    // Sinônimos: se a pessoa digitar "celular", isso traduz pra
    // categoria "Smartphones" (tabela categoria_sinonimos) e inclui
    // os produtos dessa categoria na busca também.
    let orFilter = `name.ilike.%${termoSeguro}%,brand.ilike.%${termoSeguro}%,category.ilike.%${termoSeguro}%`;
    try {
      const { data: sinonimos } = await sb.from('categoria_sinonimos')
        .select('categoria')
        .ilike('termo', termoSeguro);
      const categoriasSinonimo = [...new Set((sinonimos || []).map(s => s.categoria))];
      categoriasSinonimo.forEach(cat => { orFilter += `,category.ilike.%${cat}%`; });
    } catch (e) { /* se a tabela ainda não existir, segue a busca normal */ }

    const { data } = await sb.from('produtos')
      .select('id, name, brand, price, image_thumb_url, image_url, emoji')
      .or(orFilter)
      .limit(6);
    ...
    box.innerHTML = data.map(p => `
      ...
      <a href="${BASE}/produtos.html?q=${encodeURIComponent(q)}" class="search-suggest-vertodos">Ver todos os resultados →</a>`;
    box.classList.add('active');
  }, 250);
};

document.addEventListener('click', (e) => {
  if (!e.target.closest('.search-form') && !e.target.closest('.mobile-search-bar')) {
    document.querySelectorAll('.search-suggest').forEach(el => el.classList.remove('active'));
  }
});
...
window.toggleMobileSearch = function() {
  ...
  if (!bar) return;
  const abrindo = !bar.classList.contains('active');
  bar.classList.toggle('active', abrindo);
  if (abrindo) setTimeout(() => document.getElementById('mobile-search-input')?.focus(), 50);
};
...
async function initPage() {
  ...
  if (navEl) {
    navEl.innerHTML = navbarHTML(currentUser, isAdmin);
    aplicarTextosEm(navEl);
  }
  ...
  //
  // CORREÇÃO: o Supabase costuma disparar esse evento uma vez a mais,
  // logo depois do getSession() acima, mesmo sem nada ter mudado.
  // Sem essa checagem, a navbar (com o campo de busca) era recriada do
  // zero bem no início — se a pessoa clicasse e começasse a digitar
  // rápido, o texto digitado sumia porque o campo antigo era trocado
  // por um novo vazio. Agora só re-renderiza se o usuário realmente mudou.
  if (sb) {
    let ultimoUserId = currentUser?.id || null;
    sb.auth.onAuthStateChange((_event, session) => {
      const user = session?.user || null;
      const novoUserId = user?.id || null;
      if (novoUserId === ultimoUserId) return;
      ultimoUserId = novoUserId;
      const admin = !!(user && ADMIN_EMAILS.includes(user.email));
      if (navEl) {
        navEl.innerHTML = navbarHTML(user, admin);
        aplicarTextosEm(navEl);
      }
    });
  }
